...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const r = await window.storage.get("coloracao:resultado");
        if (r && r.value) {
          setSeason(JSON.parse(r.value));
          setStep("result");
          return;
        }
      } catch (e) {}
  ...
  async function handleFaceFile(file) {
    ...
    try {
      ...
      setSeason(parsed);
      try {
        await window.storage.set("coloracao:resultado", JSON.stringify(parsed));
      } catch (e) {}
      setStep("result");
    } catch (err) {
      console.error(err);
  ...
  async function selectPreset(name) {
    ...
    setSeason(full);
    try {
      await window.storage.set("coloracao:resultado", JSON.stringify(full));
    } catch (e) {}
    setStep("result");
  }
  ...
  async function resetAll() {
    try {
      await window.storage.delete("coloracao:resultado");
    } catch (e) {}
    setSeason(null);
    setMatchResult(null);
